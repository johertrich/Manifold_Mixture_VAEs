...
# used for the deblurring example in the paper.
...
train_dataset=BarDataset(epoch_size=n_data,start_seed=0,transform=Jittering(),centered=centered)
train_dataloader=torch.utils.data.DataLoader(train_dataset,batch_size=batch_size,shuffle=True,drop_last=True)

test_dataset=BarDataset(epoch_size=10000,start_seed=0,transform=Jittering(),centered=centered,test=True)    
...
for ng in range(num_generators):
    if large:
        decoders.append(InjectiveMultiscaleGenerator(dim_ld,sig_hd,sig_ld,utils.sampling.get_spline_latent,latent_nf=dim_ld>1,num_layers=6,latent_NF_layers=6).to(device))
    else:
...
for ng in range(num_generators):
    ...
    if dim_ld==1:
        generate_image_charts(mix_vae,plots_dir,model_name,ng,20)
    else:
